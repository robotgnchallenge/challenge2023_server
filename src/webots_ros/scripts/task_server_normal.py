            ...
            if (msg.startswith("[WEBOTS_INFO]")):
                ...
                if (self.state == "START_NAV"):
                    ...
                    if (nav_result is True):
                        ...
                        current_state += 1
                        self.state = self.task_state[current_state]
                        score_n += (1 - task_time / 600) * 40
                        score_o += (1 - task_time / 600) * 100
                        state_info = f"[{self.client_address}][NAV_SUCCESS] Successfully navigate to the target obj. Time of object search is {task_time} . Now score is {score_n}"
                        self.request.sendall(state_info.encode(FORMAT))
                    else:
                        state_info = f"[{self.client_address}][TASK_FAILED] Robot is too far to the object. Got score {score_n}."
                        self.request.sendall(state_info.encode(FORMAT))
                        save_eval_result(str(score_n), str(score_g),
                                         str(score_o))
                    ...
                    save_eval_result(str(score_n), str(score_g), str(score_o))
                    self.server.shutdown()
                    os._exit(0)
                ...
                if (self.state == "START_GSP"):
                    ...
                    if (score_grasp != 0):
                        ...
                        state_info = f"[{self.client_address}][TASK_FAILED] Failed to grasp {target_obj}. Got score {score_n}."
                        self.request.sendall(state_info.encode(FORMAT))
                        save_eval_result(str(score_n), str(score_g),
                                         str(score_o))
                        self.server.shutdown()
                ...
                if (self.state == "START_NAV_GOAL"):
                    ...
                    if (place_valid is False):
                        state_info = f"[{self.client_address}][TASK_FAILED] Robot is too far to place the object. Got score {score_n}."
                        self.request.sendall(state_info.encode(FORMAT))
                        save_eval_result(str(score_n), str(score_g),
                                         str(score_o))
                        self.server.shutdown()
                    ...
                    state_info = f"[{self.client_address}][TASK_FINISHED] Got score {score_n}."
                    self.request.sendall(state_info.encode(FORMAT))
                    save_eval_result(str(score_n), str(score_g), str(score_o))
                    self.server.shutdown()
                    os._exit(0)
...
def save_eval_result(score, score_grasp, score_obj_search):
    dict = {
        'score_n': score,
        'score_g': score_grasp,
        'score_o': score_obj_search
    }
...
if __name__ == '__main__':
    print("[STARTING] Task server (Normal) is starting ...")
